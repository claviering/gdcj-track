...
function computeRelevantStations(store, start, end) {
...
function keepTopMutating(arr, max) {
    arr.sort((a, b) => a.solution.totalMinutes - b.solution.totalMinutes ||
        a.solution.waitMinutes - b.solution.waitMinutes ||
        a.arrivalMinutes - b.arrivalMinutes);
    if (arr.length > max)
        arr.length = max;
}
function findLegTiming(track, train, startStationId, endStationId, startPos, endPos) {
    ...
    if (Math.sign(arrivePos - departPos) !== desiredDirection)
        return null;
    ...
    const departMin = (0, utils_1.parseHHmmToMinutes)(departStr);
    let arriveMin = (0, utils_1.parseHHmmToMinutes)(arriveStr);
...
function buildDirectSolutions(store, start, end) {
    ...
    for (const [cityTrackId, track] of store.loadedTracks) {
        ...
        for (const tt of track.trainTimeList) {
            ...
            const solution = {
                ...
                startStation: start,
                endStation: end,
                departTime: (0, utils_1.minutesToHHmm)(timing.departMin),
                arriveTime: (0, utils_1.minutesToHHmm)(timing.arriveMin),
                dateLabel,
                durationMinutes,
            };
    ...
    for (const candidate of candidates) {
        ...
        seen.add(keyBase);
        unique.push(item);
        if (unique.length >= MAX_DIRECT_RESULTS)
            break;
    }
    return unique;
}
...
function buildTransferSolutions(store, start, end, validStations) {
    const dateLabel = (0, utils_1.todayMonthDayLabel)();
    const solutions = [];
    ...
    for (const [cityTrackId, track] of store.loadedTracks) {
        ...
        if (posStart != null) {
            ...
            for (const s of track.stations) {
                const candidatePos = s.stationPosition;
                if (!validStations.has(s.stationName))
                    continue;
                if (candidatePos === posStart)
                    continue;
                if (direction > 0) {
                ...
                if (!transferStations.has(key))
                    transferStations.set(key, { leg1: [], leg2: [] });
    ...
    solutions.sort((a, b) => a.arrivalMinutes - b.arrivalMinutes ||
    ...
    for (const candidate of solutions) {
        ...
        seen.add(key);
        unique.push(item);
        if (unique.length >= MAX_TRANSFER_RESULTS)
            break;
    }
    return unique;
}
